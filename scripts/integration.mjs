...
const recovered=await call(`/creations/${requestId}`,{secret:teacherKey,teacher:true});assert.equal(recovered.status,200);assert.equal(recovered.data.roomId,roomId);
assert.equal((await call(`/creations/${requestId}`)).status,404);assert.equal((await call(`/creations/${requestId}`,{secret:key(),teacher:true})).status,404);pass('creation recovery requires original teacher secret and returns same room without uploads');
for(let retry=0;retry<6;retry++){const r=await fetch(base+'/api/rooms',{method:'POST',body:await form()});assert.equal(r.status,200);assert.equal((await r.json()).roomId,roomId);}pass('recovery of committed room does not exhaust new-room quota');
