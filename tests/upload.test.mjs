...
function form() {
  ...
  data.set('requestId', 'fixed-request-id');
  data.set('teacherKey', saved.teacherKey);
  data.append('photos', new Blob(['synthetic']), 'test.png');
  return data;
}
...
test('persistent network failure is bounded and gives actionable Russian error', async (t) => {
  ...
  await assert.rejects(
    ...
    (e) =>
      e.status === 0 &&
      e.message.includes('Повторить создание') &&
...
test('validation error is not retried or replaced by a network error', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (url, init) => {
    if (init.method !== 'POST') return missing();
    posts++;
    return Response.json({ error: 'Нужен ответ' }, { status: 400 });
  });
  await assert.rejects(
    uploadRoom(form(), () => {}),
    (e) => e.status === 400 && e.message === 'Нужен ответ',
  );
  assert.equal(posts, 1);
});
...
test('recovery preserves expiry and handles non-JSON gateway failures', async (t) => {
  t.mock.method(globalThis, 'fetch', async () =>
    Response.json({ error: 'Срок действия комнаты истёк' }, { status: 410 }),
  );
  ...
  await assert.rejects(
    recoverRoom('id', 'key'),
    (e) => e.status === 502 && !e.message.includes('<html>'),
  );
});
