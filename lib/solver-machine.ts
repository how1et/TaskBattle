...
export class SolverMachine {
  ...
  async show() {
    const d = this.state.draft;
    if (!d || !this.alive || d.payload || !['preparing', 'editing'].includes(this.state.phase))
      return;
    if (!d.activation) {
  ...
  async submit() {
    ...
    try {
      ...
      if (!this.alive || ready.position !== d.position || this.state.phase === 'finished') return;
  ...
  async edit(): Promise<boolean> {
    if (!this.alive || this.sending || this.finished) return false;
  ...
  async selectPhoto(file: File | null) {
    if (!this.alive || this.sending || this.finished) return;
