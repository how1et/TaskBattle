...
export async function assertAccessTime(a: Attempt, r: Room) {
...
export async function snapshot(a: Attempt, r: Room) {
  ...
  const base = {
    ...
    expiresAt: a.finished_at !== null ? resultExpiry(a) : r.expires_at,
...
export async function start(req: Request, r: Room, ip: string) {
  ...
  const now = Date.now();
  await sql(
    'INSERT INTO attempts (id,room_id,secret_hash,order_json,position,started_at,task_started_at,timing_version) SELECT ?,?,?,?,0,?,0,2 WHERE (SELECT count(*) FROM attempts WHERE room_id=?)<? AND EXISTS(SELECT 1 FROM rooms WHERE id=? AND expires_at>?) ON CONFLICT(id) DO NOTHING',
    data.attemptId,
    r.id,
    secretHash,
    JSON.stringify(ids),
...
export async function ready(req: Request, a: Attempt, r: Room) {
  ...
  await sql(
    'UPDATE attempts SET ready_id=?,task_started_at=?,started_at=CASE WHEN position=0 THEN ? ELSE started_at END WHERE id=? AND position=? AND ready_id IS NULL AND finished_at IS NULL AND EXISTS(SELECT 1 FROM rooms WHERE id=attempts.room_id AND expires_at>?)',
    data.readyId,
    data.startedAt,
    data.startedAt,
    a.id,
...
export async function cancelAnswer(req: Request, a: Attempt, r: Room) {
  ...
  await sql(
    'INSERT INTO cancelled_answers(attempt_id,request_id) SELECT id,? FROM attempts WHERE id=? AND position=? AND finished_at IS NULL AND NOT EXISTS(SELECT 1 FROM answers WHERE attempt_id=? AND request_id=?) ON CONFLICT DO NOTHING',
    data.requestId,
    a.id,
    data.position,
    a.id,
  ...
  return {
    cancelled: cancelled && fresh.position === data.position,
...
export async function submit(req: Request, a: Attempt, r: Room, trace: RequestTrace) {
  ...
  if (
    ...
    (!Number.isSafeInteger(data.durationMs) ||
      data.durationMs < 0 ||
      data.durationMs > LIMITS.roomTtlMs ||
      !validId(data.readyId))
  )
  ...
  if (file && bytes && mime && digest) {
    ...
    await sql(
      "INSERT INTO blobs(key,expires_at,state) VALUES (?,?,'live') ON CONFLICT(key) DO NOTHING",
      solutionKey,
      Date.now() + 3600000,
    ).run();
  ...
  try {
    await db().batch([
      ...
      sql(
        `UPDATE blobs SET expires_at=MAX(expires_at,?) WHERE key IN(SELECT file_key FROM tasks WHERE room_id=?) AND EXISTS(SELECT 1 FROM attempts WHERE id=? AND finished_at IS NOT NULL)`,
        now + RESULT_TTL,
        r.id,
        a.id,
      ),
      sql(
        `UPDATE blobs SET expires_at=MAX(expires_at,?) WHERE key IN(SELECT solution_key FROM answers WHERE attempt_id=?) AND EXISTS(SELECT 1 FROM attempts WHERE id=? AND finished_at IS NOT NULL)`,
        now + RESULT_TTL,
        a.id,
        a.id,
      ),
